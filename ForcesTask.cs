...
public class ForcesTask
{
	...
	{
		return x=>new Vector(x.Velocity.X/Math.Abs(x.Velocity.X), x.Velocity.Y / Math.Abs(x.Velocity.Y))
    }
	...
	{
		return x =>new Vector(forces.ToList().Select(r => r(x)).Select(p=>p.X).Sum(), forces.ToList().Select(r => r(x)).Select(p => p.Y).Sum());
